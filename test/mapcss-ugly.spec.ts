...
describe('Test mapCss function with uglified css file from Font Awesome module', () => {
    ...
    before('load css file to test', () => {
        try {
            fileContent = fs.readFileSync(require.resolve('@fortawesome/fontawesome-free/css/fontawesome.css'), 'utf8');
        } catch (err) {
            throw err;
        }
    });
    ...
    it('has expected keys and values', () => {
        const assertPropExistsWithValue = (map, key, value) => {
            ...
            assert.strictEqual(
                map[key],
                value,
                `Values for key "${key}" do not match.` + ` Expected: "${value}" (charcode ${value.charCodeAt(0)}),` + ` actual: "${map[key]}" (charcode ${map[key].charCodeAt(0)})`
            );
        };
